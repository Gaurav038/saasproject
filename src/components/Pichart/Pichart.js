...
const Pichart = () => {
    ...
    const renderCustomizedLabel = ({ value, cx, cy, midAngle, innerRadius, outerRadius, percent, index }) => {
        ...
        return (
            <text x={x} y={y} fontWeight={index === selectedSliceIndex ? 700 : 500} fill="Black" textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central">
                {value}%
            </text >
        );
    };
    ...
    const renderCustomizedLabelLine = ({
        ...
        const arrowHeadX = innerBorderX + (outerRadius / 2.22 - innerRadius) * Math.cos(-midAngle * RADIAN);
        const arrowHeadY = innerBorderY + (outerRadius / 2.22 - innerRadius) * Math.sin(-midAngle * RADIAN);

        return (
            <polygon
                points={`${arrowHeadX - 8},${arrowHeadY + 8} ${arrowHeadX},${arrowHeadY} ${arrowHeadX + 8},${arrowHeadY + 8}`}
                fill="#000"
            />
        );
    };
                ...
                >
                    {data.map((entry, index) => (
                        <Cell
                            ...
                            stroke={index === selectedSliceIndex ? "rgba(51, 51, 51, 0.10)" : "rgba(51, 51, 51, 0.10)"}
                            style={
                    ...
                    <Label
                        width={50} value={labelName} position="center" className='label-center' fontWeight={500} fill="black" fontSize='18px'
                    />
                    <Tooltip />
                </Pie>
            </PieChart >
